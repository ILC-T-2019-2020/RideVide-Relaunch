...
def index(request):
    if request.user.is_authenticated():
        ...
        my_rides = request.user.profile.ride_set.filter(
...
def browse_detail(request, ride_id):
    ...
    if ride.date < today:
        ...
        archived = False
    return render(request, "ridevide_app/browse_detail.html",
                  dict(ride=ride, ride_id=ride_id, archived=archived))
...
def add_user_to_ride(request, ride_id):
    ...
    if request.method == 'POST':
        if eligibleForRide(request, ride.date, ride.time):
            ...
            return render(request, "ridevide_app/browse_detail.html",
                          dict(ride=ride, ride_id=ride_id, error=error))
...
def browse_rides(request, rides, heading):
    ...
    return render(request, "ridevide_app/browse_rides.html",
                  dict(heading=heading, formatted_rides=formatted_rides))


@login_required(login_url='/')
def browse_from_campus(request):
    today = datetime.date.today().strftime('%Y-%m-%d')
    from_campus_rides = Ride.objects.filter(from_campus=True,
                                            date__gte=today).order_by('date')
...
def browse_to_campus(request):
    today = datetime.date.today().strftime('%Y-%m-%d')
    to_campus_rides = Ride.objects.filter(from_campus=False,
                                          date__gte=today).order_by('date')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if date < today:
                ...
                return render(
                    request, "ridevide_app/add_rides.html",
                    dict(form=form,
                         heading="Add Ride from Campus",
                         error=error))
            ...
            return render(request, "ridevide_app/add_rides.html",
                          dict(form=form, heading="Add Ride from Campus"))
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if eligibleForRide(request, date, time):
                ...
                return render(
                    request, "ridevide_app/add_rides.html",
                    dict(form=form, heading="Add Ride to Campus", error=error))
        else:
            return render(request, "ridevide_app/add_rides.html",
                          dict(form=form, heading="Add Ride to Campus"))
    else:
...
def stats(request):
    ...
    return render(
        request, "ridevide_app/stats.html",
        dict(total_rides=total_rides,
             completed_rides=completed_rides,
             upcoming_rides=upcoming_rides,
             users=users,
             completed_rides_with_more_than_2_riders=
             completed_rides_with_more_than_2_riders,
             completed_rides_with_more_than_3_riders=
             completed_rides_with_more_than_3_riders,
             completed_rides_with_4_riders=completed_rides_with_4_riders))
